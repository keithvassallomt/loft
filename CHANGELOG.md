...
- **Loft no longer claims it isn't allowed to start at login when it is.** If your autostart entry was a symbolic link — which it is if you keep your config in a dotfiles repository — the Flatpak build could not read through it and concluded there was no entry, so the settings page warned that "On login" would not take effect on every launch, while Loft was in fact starting at login perfectly well. Loft now checks that the entry is *there* rather than that it can read through it.
- **The advice given when autostart really is blocked now matches your desktop.** It told everyone to turn on "Run in Background" in Settings → Apps → Loft, which only exists on GNOME. On KDE, Hyprland, sway and the rest you now get a command you can copy, and outside Flatpak you get the actual problem — the entry could not be written — instead of a permission setting that has nothing to do with it.
- **Services now come back on their own after standby or a spell offline.** A service whose connection died while the machine was asleep, or while the network was down, would sit there showing stale messages and a stale unread count until you thought to reload it by hand. Loft now notices when it has woken up or come back online, gives each service a moment to reconnect by itself, and reloads only the ones that are still receiving nothing. It will not reload a service you are on a call in, one you are part-way through typing a message in, or the same service twice in a row.
- **A settings file Loft could not read is no longer replaced with an empty one.** If reading `config.json` failed for any reason — a truncated file, a permissions problem, a disk error — Loft could not tell that apart from a first run, so it started as if freshly installed and then saved that empty state back over your real settings a few seconds later, the moment anything moved a window. Every service, every per-service setting and every global setting were gone, while the logins they belonged to sat untouched on disk. Loft now tells the two cases apart: an unreadable settings file is reported to you and left strictly alone, with saving switched off for that session so nothing can overwrite it. Loft also keeps a known-good backup and offers to restore from it, writes settings by replacing the file in one step so a crash or a logout can never leave a half-written one behind, and no longer lets a second launch touch the file at all.
- Development tooling: `npm run dev` cloned the installed app's profile to inherit real logins, including WhatsApp's and Element's. Those are linked-device identities rather than logins, so the clone was a second copy of one device; running it forked that device's encryption state and made messages go missing and sends fail silently in the real install. Those two are no longer cloned.
