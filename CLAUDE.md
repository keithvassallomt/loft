...
**GNOME system-DND under Flatpak comes from the Shell helper, not from `gsettings`.** Backend selection is `selectSystemDndBackend` (`src/main/notifications/systemDnd.ts`): KDE → `Inhibited`, unsandboxed GNOME → `gsettings`, **GNOME-under-Flatpak → the helper's `SystemDnd` property**, anything else → no-op.
