...
> **Known limitation — GNOME system-DND is not detected under Flatpak.** The GNOME backend shells out to `gsettings`, and a modern Flatpak sandbox has no path to the host's dconf: it has no dconf grant (and even with one, current flatpak removed the dconf bridge, so `gsettings` reads only the empty in-sandbox schema defaults), and the XDG **Settings portal — the one mechanism that does reach the host — exposes only a curated set of namespaces that does *not* include `org.gnome.desktop.notifications`** (unlike `org.freedesktop.appearance`, which is why the live-theme feature *can* use the portal). `Inhibited` is a KDE-only property; GNOME's notification server does not implement it. So on GNOME-under-Flatpak, Loft's system-DND gate is inert. This is **mostly cosmetic**: Loft's notifications go through `org.freedesktop.Notifications`, and GNOME Shell suppresses the *banner* for anything sent while DND is on regardless of the sender, so the user still sees no banner. The residual gap is the in-page notification *sound* (gated on the same undetected signal), and that Loft keeps emitting notifications into GNOME's message tray. Loft's own global/per-service DND is the in-app substitute. Not worth reintroducing `flatpak-spawn --host` (the sandbox escape the rewrite removed) to close. Verified against xdg-desktop-portal-gnome's exposed namespaces on Fedora, 2026-07-25. Because the backend is inert there, `defaultSystemDndDeps` resolves GNOME-under-Flatpak to the **no-op** rather than to the gsettings backend: the sandboxed read would report a confident "DND off" (the schema default) where `null`/unknown is the honest answer, and the `gsettings monitor` child it spawned was a real liability at shutdown (see Session end below).
