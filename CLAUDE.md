...
5. **Notifications** (`src/main/notifications/`) — a hand-rolled `dbus-next` client for `org.freedesktop.Notifications`, kept on a persistent connection (KDE closes notifications when the sender disconnects). **Binding to that name is retried, not attempted once** (`createNotificationServer`, shared backoff in `src/main/dbusRetry.ts`): `getProxyObject` introspects the destination, so it fails outright while the name has no owner, and at login it routinely has none for a second or two — autostart fires Loft alongside the shell, not after it. Taking that single failure as final left `server` undefined and dropped every notification for the rest of the session, with badges and the web apps' own ding still working, so nothing looked broken. Measured on a Hyprland/Caelestia login (2026-09-09): Loft up at 07:15:57, the shell restarting itself at 07:16:00.455, Loft's one attempt landing at 07:16:01.114 inside that gap. The tray had had this cure for `StatusNotifierWatcher` since the Rust build; this client had not. One *successful* bind is then kept for the process lifetime — dbus-next addresses calls to the well-known name and filters signals against the owner it tracks from `NameOwnerChanged`, so a server that later restarts is followed with no rebind, and rebinding would leave the old proxy's `ActionInvoked` listener attached and route every click twice. Avatars are resolved in the main process via each service's own partition session (`session.fetch(url)`, so **cookie**-authenticated avatars — NextCloud Talk's — work), cached on disk for about an hour. **Element is the exception and its avatars are read in the page instead**: Synapse serves authenticated media, and the access token lives in the page (Element's service worker attaches it) rather than in a cookie, so the URL its `<img>` renders returns `404 application/json` to main no matter which session fetches it. Both avatar paths handle this the same way — `bridge.ts` inlines the notification icon to a data URI, and `CapturedConversation.inlineAvatar` does the same for bubbles. Measured against a real homeserver, 2026-07-27. Clicking a notification (`ActionInvoked`) focuses the service window and navigates to the conversation.
...
Both property-backed backends share one implementation (`propertyDeps`): read the property, follow `PropertiesChanged`, tolerate an unreadable initial read (a server may be mid-startup) but keep the change stream, and release the bus connection on `stop()` — which the older KDE-only copy never did. The **connect** is retried on the shared `dbusRetry` backoff, for the same reason the notification client is (§5): `Inhibited` lives on the very bus name Loft raced and lost at a measured login, and the helper's property is published by an extension that loads on its own schedule. The retry is deliberately **bounded** to the schedule (~30s, six attempts) rather than held at the maximum for ever like the tray's — a desktop whose server has no such property is the common case, and each attempt opens and closes its own session-bus connection. A failed *read* on an attached server never retries the connect: a server answering `No such property` has answered.
...
10. **Config store** (`src/main/configStore.ts`) — the single gate on `config.json`. Its reason to exist is one invariant that cannot be enforced call-site by call-site: **a failure to READ an existing config must never become writable default state.** `loadConfig` used to swallow every error and return `{services:{}}`, indistinguishable from a first run, so one unreadable read had its defaults committed by the next debounced window-move flush — every service, every per-service setting and the whole top level replaced with `{"services":{}}`, permanent loss out of a possibly transient failure. `loadConfigResult` now distinguishes **loaded / missing / recovered / error** (`missing` means no primary *and* no backup, i.e. a real first run); the store turns `error` into `writable: false`, which makes `save()` inert for all ~30 callers at once. Migration and autostart reconciliation are additionally gated on `writable` explicitly — not because `save()` would leak, but because both act on state *outside* `config.json` (a `configVersion` stamp on nobody's install; deleting the login `.desktop` entry), which no amount of not-saving would undo. `saveConfig` is atomic: serialize first, write a temp file beside the target, `fsync`, then `rename()` over it — the old direct `writeFileSync` truncated the live file before writing a byte, so a crash, a logout (~21ms of budget under Flatpak; see `shutdown.ts`), a full disk or a concurrent reader could all see an empty or half-written config.
    ...
    Read-only is the right state to *start* in and the wrong state to be stuck in, so `startFresh()` is the way out: it sets the unusable file aside and flips the store writable, and is called only from the dialog that asks. A failure to move the file leaves the store read-only — becoming writable would let the next save rename a fresh config over the only copy of what went wrong.

    **A config wipe is survivable because the config is not where the logins are.** `src/main/adopt.ts` scans `Partitions/` for session directories no config entry claims and, when the config lists nothing, offers to re-adopt them (`adoptInstance`, which differs from `addInstance` in exactly one load-bearing way: it keeps the id the partition is bound to, rather than allocating the lowest free one and handing the account an empty session). A `serverRequired` kind has no usable URL without its `customUrl`, so Talk's server is guessed from the cookie hosts in its own partition. Nothing here acts unasked: an unclaimed partition is equally what a lost config and a "remove but keep my login data" look like, and only the user can tell them apart. A decline is recorded as `adoptDeclined` and dropped the moment any service exists, so it cannot outlive the question it answered.

    > Both real config losses to date were caused by a build that predates this file — a Flatpak keeps its old deployment alive for an already-running process, so the code that wrecks a config at logout can be one that was replaced days earlier. The protections here cannot apply retroactively, and a first launch on a fixed build has no backup yet (it is promoted from bytes that parsed, which an unreadable config never produces). Recovery from the partitions is the floor under that.
...
~/.config/loft/
  ...
  config.json.corrupt               # an unreadable primary, set aside (not overwritten) when the
                                    # backup took over from it, or when the user chose Start
                                    # Fresh. A second failure lands on `.corrupt.<timestamp>`
                                    # rather than over the first: that file is the only copy of
                                    # what went wrong, and the settings it holds may still be
                                    # mineable by hand.
                                    ...
                                    #  trayBackend, globalDnd, railOrder, grid and adoptDeclined at the top level)
