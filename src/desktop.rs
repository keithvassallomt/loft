...
/// Derive an `(origin, match-pattern)` pair from a custom service URL.
///
/// `https://chat.example.com/foo` → `("https://chat.example.com", "https://chat.example.com/*")`.
/// Returns `None` for anything that isn't a plain `http(s)://host` URL.
fn origin_and_pattern(url: &str) -> Option<(String, String)> {
...
/// Collect `(origin, pattern, service_name)` for every service that has a
/// `custom_url` configured. Built-in URLs are already in the manifest, so only
/// overrides need to be injected.
fn custom_origin_overrides() -> Vec<(String, String, String)> {
    let mut out = Vec::new();
    for svc in crate::service::ALL_SERVICES {
        let cfg = ServiceConfig::load(&svc.name).unwrap_or_default();
        if let Some(url) = cfg.custom_url.as_deref() {
            if let Some((origin, pattern)) = origin_and_pattern(url) {
                out.push((origin, pattern, svc.name.to_string()));
            }
        }
    }
    out
...
/// Inject custom-origin match patterns into the extension manifest so Chrome
/// grants host permissions and injects the content scripts on a self-hosted
/// service (e.g. a custom Element Web instance). Loft deploys its own
/// extension, so the manifest Chrome loads is fully under our control — adding
/// the origin here makes it a *granted* permission at load time, with no
/// runtime permission prompt.
fn template_manifest(manifest_src: &str, patterns: &[String]) -> Result<String> {
    if patterns.is_empty() {
        return Ok(manifest_src.to_string());
    }
    let mut manifest: serde_json::Value =
        serde_json::from_str(manifest_src).context("Failed to parse extension manifest")?;

    let add_patterns = |arr: &mut Vec<serde_json::Value>| {
        for p in patterns {
            let v = serde_json::Value::String(p.clone());
            if !arr.contains(&v) {
                arr.push(v);
            }
        }
    ...
    if let Some(hp) = manifest
        .get_mut("host_permissions")
        ...
        .and_then(|v| v.as_array_mut())
    {
        for entry in scripts.iter_mut() {
            if let Some(matches) = entry.get_mut("matches").and_then(|v| v.as_array_mut()) {
                add_patterns(matches);
            }
        }
    }

    serde_json::to_string_pretty(&manifest).context("Failed to serialize templated manifest")
}

/// Deploy the Chrome extension files to ~/.local/share/loft/extension/.
///
/// The manifest is templated with any configured `custom_url` origins, and a
/// generated `loft-overrides.js` exposes the `origin → service` map so the
/// content script / service worker can identify a custom domain as its service.
pub fn deploy_extension() -> Result<()> {
    ...
    let overrides = custom_origin_overrides();
    let patterns: Vec<String> = overrides.iter().map(|(_, p, _)| p.clone()).collect();

    let manifest = template_manifest(include_str!("../extension/manifest.json"), &patterns)?;

    // Generated origin → service map, consumed by content.js (shared isolated
    // world) and background.js (via importScripts). Always written — even empty
    // — so background.js can importScripts it unconditionally.
    let override_map: serde_json::Map<String, serde_json::Value> = overrides
        .iter()
        .map(|(origin, _, svc)| (origin.clone(), serde_json::Value::String(svc.clone())))
        .collect();
    let overrides_js = format!(
        "// Generated by Loft — do not edit. Maps custom service origins to names.\n\
         self.__loftOverrides = {};\n",
        serde_json::to_string(&serde_json::Value::Object(override_map))?
    );
    ...
    std::fs::write(ext_dir.join("manifest.json"), &manifest)
        .context("Failed to write templated manifest.json")?;
    std::fs::write(ext_dir.join("loft-overrides.js"), &overrides_js)
        .context("Failed to write loft-overrides.js")?;

    tracing::debug!(
        "Deployed extension to {} ({} custom origin override(s))",
        ext_dir.display(),
        overrides.len()
    );
    Ok(())
...
mod tests {
    ...
    #[test]
    fn test_origin_and_pattern() {
        assert_eq!(
            origin_and_pattern("https://chat.example.com/"),
            Some((
        ...
        assert_eq!(
            origin_and_pattern("https://chat.example.com/element/#/welcome"),
            Some((
                "https://chat.example.com".to_string(),
                "https://chat.example.com/*".to_string()
            ))
        ...
        assert_eq!(
            origin_and_pattern("http://localhost:8080/foo"),
            Some((
                "http://localhost:8080".to_string(),
                "http://localhost:8080/*".to_string()
            ))
        );
        // Rejected: non-http scheme, missing host, garbage.
        assert_eq!(origin_and_pattern("ftp://example.com"), None);
        assert_eq!(origin_and_pattern("https:///nohost"), None);
        assert_eq!(origin_and_pattern("not a url"), None);
    }

    #[test]
    fn test_template_manifest_adds_patterns() {
        let src = include_str!("../extension/manifest.json");
        let patterns = vec!["https://chat.example.com/*".to_string()];
        let out = template_manifest(src, &patterns).unwrap();
        let v: serde_json::Value = serde_json::from_str(&out).unwrap();

        // Added to host_permissions.
        let hp = v["host_permissions"].as_array().unwrap();
        assert!(hp.iter().any(|p| p == "https://chat.example.com/*"));

        // Added to every content_scripts entry's matches.
        for entry in v["content_scripts"].as_array().unwrap() {
            let matches = entry["matches"].as_array().unwrap();
            assert!(matches.iter().any(|p| p == "https://chat.example.com/*"));
        }
    }

    #[test]
    fn test_template_manifest_empty_is_unchanged() {
        let src = include_str!("../extension/manifest.json");
        assert_eq!(template_manifest(src, &[]).unwrap(), src);
    }
}
